...
class TestBuildBpPanelObservation:
    ...
    def test_status_both_final(self):
        obs = _build_bp_panel_observation(
            self._sys(status="F"), self._dia(status="F"), "Patient/2", None
        )
        assert obs.status == "final"

    def test_status_one_preliminary(self):
        obs = _build_bp_panel_observation(
            self._sys(status="P"), self._dia(status="F"), "Patient/2", None
        )
        assert obs.status == "preliminary"
...
class TestHl7DatetimeToIso:
    def test_full_datetime(self):
        assert _hl7_datetime_to_iso("20260420103000") == "2026-04-20T10:30:00+10:00"

    def test_date_only(self):
        assert _hl7_datetime_to_iso("20260420") == "2026-04-20"

    def test_short_string(self):
        assert _hl7_datetime_to_iso("2026") == "2026"
